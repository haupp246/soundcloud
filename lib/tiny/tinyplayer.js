...
function Track( ID, audioTrack, nextCallback, current, showwaveform ) {
	...
	function showPlayer() {
		...
		divContent    = '<audio id="'+audioIDName+'" class="audio-link" data-song-id="'+audioTrack.songID+'" data-view-count="'+audioTrack.viewCount+'">Your browser doesn\'t support HTML5 audio tag.</audio>';
		...
		divContent    += '</div>';
		divContent    += '</div>';
		divContent    += '<a class="link-wrapper" href="/soundcloud/view/playsong.php?id='+audioTrack.songID+'">';
		...
		divContent    += '</a>';
		divContent    += '</div>';
		divContent    += '<div class="stat-wrapper"><span class="glyphicon glyphicon-play" aria-hidden="true" title="'+audioTrack.viewCount+' times played"><div class="view-count">'+audioTrack.viewCount+'</div></span><span class="glyphicon glyphicon-heart" aria-hidden="true" title="'+audioTrack.likeCount+' likes"><div class="like-count">'+audioTrack.likeCount+'</div></span></div>';
		...
		window.addEventListener('DOMContentLoaded', function() {
			...
			function kbShortcuts(e) {
				switch(e.keyCode) {
					...
					case 32:
							if($("#write-comment-body").is(':focus') || $("#search-box").is(':focus')){
								break;
							}else {
                                e.preventDefault();
