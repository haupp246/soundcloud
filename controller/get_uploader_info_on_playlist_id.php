...
$playlist_id = json_decode($_POST['playlistID']);
...
from user as u,
  (
    select count(songID) as totalsongs from song where userID in (select userID from playlist where playlistID = $playlist_id)
    ) as usersong
where userID in (select userID from playlist where playlistID = $playlist_id)";
